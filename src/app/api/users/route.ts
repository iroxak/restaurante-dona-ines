...
export async function GET() {
  try {
    ...
    const users = await db.user.findMany({ select: { id: true, username: true, role: true, createdAt: true }, orderBy: { createdAt: "asc" } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const user = await db.user.create({ data: { username, password: hashedPassword, role: role || "user" }, select: { id: true, username: true, role: true } });
