...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await db.quotation.update({
      where: { id },
      data: {
        date: date ?? existing.date, client: client !== undefined ? JSON.stringify(client) : existing.client,
        notes: notes !== undefined ? notes : existing.notes, status: status ?? existing.status,
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    const db = await getDb();
    const quotation = await db.quotation.findUnique({ where: { id }, include: { createdBy: { select: { username: true } } } });
    if (!quotation) return NextResponse.json({ error: "Cotización no encontrada" }, { status: 404 });
